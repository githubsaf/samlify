  ...
  return {
    ...
    verifySignature(xml: string, opts: SignatureVerifierOptions) {
      ...
      if (assertionSignatureNode.length === 1) {
        const verifiedAssertionInfo = extract(
          assertionSignatureNode[0].toString(),
          [
            {
              key: "refURI",
     ...
     */
    async constructMessageSignature(
      octetString: string,
      key: string,
      passphrase?: string,
      isBase64?: boolean,
      ...
      // Convert octetString to buffer
      const octetStringBuffer = Buffer.from(octetString);
    ...
    encryptAssertion(sourceEntity, targetEntity, xml?: string) {
      ...
      return new Promise<string>((resolve, reject) => {
        ...
        if (sourceEntitySetting.isAssertionEncrypted) {
          ...
          xmlenc.encrypt(
            rawAssertionNode.toString(),
            {
              // use xml-encryption module
              rsa_pub: Buffer.from(publicKeyPem), // public key from certificate
              pem: Buffer.from(
                `-----BEGIN CERTIFICATE-----${targetEntityMetadata.getX509Certificate(
                  certUse.encrypt
                )}-----END CERTIFICATE-----`
              ),
            ...
            (err, res) => {
              if (err) {
                console.error(err);
                return reject(
                  new Error("ERR_EXCEPTION_OF_ASSERTION_ENCRYPTION")
                );
              }
              if (!res) {
